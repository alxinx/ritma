...
;(function () {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        async function triggerSearch() {
            ...
            try {
                ...
                searchTbody.innerHTML = json.data.map(m => {
                    ...
                    const favIcon = m.enFavoritos ? 'heart_minus' : 'favorite';
                    const favTitle = m.enFavoritos ? 'Quitar de Favoritos' : 'Agregar a Favoritos';
                    const favColor = m.enFavoritos ? 'text-red-400' : 'text-white/30';
                    const favBtn = `<button class="btn-fav-search p-1.5 ${favColor} hover:text-red-400 transition-colors" title="${favTitle}" data-id="${m.idMultimedia}" data-fav="${m.enFavoritos ? '1' : '0'}"><span class="material-symbols-outlined text-[18px]">${favIcon}</span></button>`;
                ...
                // Bind favorito toggle buttons
                searchTbody.querySelectorAll('.btn-fav-search').forEach(btn => {
                    btn.addEventListener('click', () => handleFavoritoToggle(btn));
                });
        ...
        // ==========================================
        // FAVORITO TOGGLE (search results)
        // ==========================================
        async function handleFavoritoToggle(btn) {
            const idMultimedia = btn.dataset.id;
            try {
                btn.disabled = true;
                btn.style.opacity = '0.5';
                ...
                if (data.ok) {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: data.enFavoritos ? 'Agregado a Favoritos' : 'Eliminado de Favoritos',
                        timer: 1500,
                        showConfirmButton: false,
                        background: '#0a0a0c',
                        color: '#fff'
                ...
                console.error('Error favorito toggle:', err);
